...
fecha: 2026-09-03
tipo: documentacion
estado: v1.12 — 68 historias con criterios de aceptación, agrupadas en épicas por rol. Numeración heredada del spec 06; HU-24 en adelante son nuevas. v1.12 (2026-09-14, DOJO-43) agrega HU-18b: pago presencial (efectivo, tarjeta, transferencia) para admin/owner, confirmado contra GymBase v1 como feature real que se había perdido en el rediseño -- surgió de la misma pregunta de Marcos que llevó a revertir HU-41 ("¿los admins pueden registrar pagos presenciales?"). Reusa `asignar_plan_miembro` y `aprobar_comprobante` sin tabla nueva, solo columna `payment_method` en `payment_proofs`. v1.10 (2026-09-14, DOJO-41) agrega CA-07/CA-08 a HU-16: modo "ranking" (top 1 / top 3) como alternativa a la meta compartida original, nacido en vivo durante una sesión de QA de flujos, sobre la misma tabla de progreso ya construida en DOJO-28 sin tocarla. v1.11 (2026-09-14) HU-41 se construyó como DOJO-42 (esconder Gestión/Negocio del menú) y se revirtió el mismo día: Marcos señaló que staff/owner ya llegan a cualquier pantalla de alumno sin bloqueo, así que esconder el menú no resolvía nada -- el gap real es que en pantallas compartidas (ficha, mediciones) el staff sigue viendo sus propios controles (Editar, Registrar medición, Asignar rango) que un alumno no tiene. HU-41 queda sin construir hasta abordar eso, no el menú. v1.1 agrega progresión por disciplina, ficha del alumno y módulos opcionales (revisión de mockups); v1.2 agrega lo que salió del repaso de GymBase v1: sesión de sparring con cronómetro, clases recurrentes, anuncios, notificaciones por correo y proyección de torneos. v1.4 (2026-09-03) corrige HU-05/06/07/07c/08/09 contra lo que DOJO-9 terminó construyendo: sin fecha propuesta, el reto se decide por rounds ganados (no suma de puntos) con KO/sumisión/decisión por round, HU-07b (confirmación del rival) removida, preparación de 10s antes de cada round. v1.5 (2026-09-06) agrega la épica "Torneos" (HU-42 a HU-44, bocetada y sin priorizar) tras aclarar con Marcos que Torneos y Peleas oficiales (HU-10/11/11b) son features distintas — HU-38 (proyección) se reubicó ahí, vivía suelta bajo Configuración sin ninguna otra HU de torneos escrita todavía. v1.6 (2026-09-12) acota HU-16 (Challenges) antes de arrancar DOJO-28: el tipo "rutina" se sacó de alcance (no existe ninguna feature de rutinas en el producto, sin fuente de datos para medir cumplimiento) y la UI se renombra a "Desafíos" para no chocar con "reto" de Sparring. v1.7 (2026-09-13, DOJO-32) agrega HU-43b: después de cerrar DOJO-30 con cruces manuales (HU-43 CA-03, confirmado dos veces con Marcos), pidió explícito un cuadro de eliminación con bracket automático y avance de rondas, referencia visual GymBase v1 -- HU-43 CA-03 queda como el modo "manual" (default), HU-43b agrega el modo "bracket" como alternativa elegida al crear el torneo, sin poder cambiar después. HU-44 CA-02 se amplía con marcador de puntos opcional (no reemplaza elegir ganador con un toque) y CA-04 nueva (deshacer un resultado). v1.8 (2026-09-13, tras cerrar DOJO-31) agrega HU-45, bocetada y lista para ticket: Marcos vio la card de cruce manual de la proyección de Torneos y pidió reusarla para registrar el cruce de un criterio de tipo Combate en un evento de promoción. Las dos preguntas de diseño se resolvieron el mismo día: (a) el ganador del cruce lo decide siempre el admin con un toque, el marcador de puntos solo sugiere -- mismo criterio que HU-44 CA-02 de Torneos, porque una sumisión/KO puede pesar más que ir ganando en puntos; (b) el resultado del cruce y la nota del criterio son datos independientes -- "la pelea es la pelea", el admin/owner sigue cargando la nota aparte, sin que el cruce se la sugiera ni se la fije. v1.9 (2026-09-13, DOJO-36) acota HU-37 CA-01 antes de construirla: el teléfono se queda en Mi ficha (`member_files.phone`, vertical), Editar mi perfil (core) queda solo con nombre y foto -- evita dos lugares editando el mismo dato. CA-04 nueva: la foto es un bucket privado con URL firmada, mismo criterio que comprobantes/adjuntos.
...
- CA-03: **Ampliado (2026-09-13, DOJO-32):** esto describe el modo **manual** del torneo (el default). Armo los cruces a mano, un participante contra otro — no hay bracket generado automáticamente ni avance automático de rondas EN ESTE MODO. Un torneo que se crea en modo **bracket** (nuevo, ver HU-43b) sí arma el cuadro solo y avanza los ganadores de ronda en ronda. El modo se elige una sola vez al crear el torneo y no se puede cambiar después.
...
**HU-43b: Generar el cuadro automáticamente (bracket)** *(nueva — 2026-09-13, DOJO-32, a pedido de Marcos tras ver el modo manual en vivo — referencia visual y funcional: el bracket de GymBase v1)*
Como admin, quiero que el torneo arme su propio cuadro de eliminación simple con líneas de avance entre rondas, para no tener que emparejar manualmente cada cruce yo mismo.
- CA-01: Al crear el torneo elijo su formato: manual (HU-43) o bracket. No se puede cambiar después de creado.
- CA-02: En un torneo bracket, puedo asignarle un número de siembra (seed) a cada participante antes de generar el cuadro; los que no tienen seed se ordenan por orden de ingreso. Los seeds 1 y 2 quedan siempre en mitades opuestas del cuadro.
- CA-03 **[servidor]**: "Generar bracket" arma el cuadro completo de una sola vez a partir de los participantes ya cargados — el tamaño es siempre la próxima potencia de 2 (4, 8, 16...) igual o mayor a la cantidad de participantes, con pases directos (bye) para las posiciones que sobran. No se puede generar dos veces ni agregar participantes nuevos después de generado.
- CA-04 **[servidor]**: Al registrar el ganador de un cruce, avanza solo a la posición que le corresponde en la ronda siguiente. Si se corrige un resultado ya cargado, todo lo que dependía de él en las rondas siguientes se limpia en cascada, no queda un cuadro inconsistente.
- CA-05: El cuadro se ve como un diagrama de eliminación de verdad — una columna por ronda, líneas conectando cada cruce con la posición a la que avanza el ganador, nombre de la ronda (Final, Semifinal, Cuartos...), y un cartel de campeón cuando termina la final.
- CA-06: Sin categorías por peso/rango (divisiones) ni partido por el tercer puesto en esta versión — un solo cuadro por torneo.
...
- CA-02: Puedo registrar resultado y método del cruce (mismo vocabulario que una pelea oficial, HU-10), quedando reflejado en la card. **Ampliado (2026-09-13, DOJO-32):** también puedo llevar un marcador de puntos de ambos lados mientras se juega el cruce (mismos incrementos de la disciplina que ya usa Sparring) — el marcador es opcional y solo sugiere el ganador, elegirlo sigue siendo un toque aparte, nunca automático por puntaje solo.
...
- CA-04 **[servidor]**: Puedo deshacer un resultado ya cargado (sin esto no existía forma de corregir un error de carga); en un torneo bracket, deshacer también limpia en cascada lo que dependía de ese resultado (HU-43b CA-04).
...
- CA-02: Para un criterio de combate, en la grilla de calificación (HU-13 CA-01) armo un cruce entre dos candidatos del evento — misma card de "vs" que ya usa Torneos en modo manual (HU-44 CA-01/CA-02), sin bracket ni eliminación.
- CA-03: **Resuelto (2026-09-13) — cómo se decide el ganador del cruce.** Mientras se juega el cruce llevo un marcador de puntos de ambos lados (mismos incrementos de la disciplina que ya usa Sparring/Torneos bracket, HU-44 CA-02), pero el marcador solo **sugiere** un ganador, nunca lo decide solo: elegir quién ganó sigue siendo un toque aparte del admin, igual que en Torneos. Alguien puede ir arriba en puntos y aun así perder por sumisión, KO o decisión — el puntaje importa hasta que un método puntual lo hace irrelevante.
- CA-03b: **Resuelto (2026-09-13) — el puntaje de la pelea y la nota del criterio son dos datos independientes.** Registrar el resultado del cruce (ganador, método) **no** completa ni sugiere la nota del criterio: "la pelea es la pelea". El admin u owner carga esa nota aparte, con el mismo puntaje máximo y stepper que ya usa cualquier otro criterio (HU-13 CA-01) — no se le asigna el máximo al ganador ni cero al perdedor automáticamente, y puede ajustarla en cualquier momento antes de resolver al candidato, igual que hoy.
- CA-04 **[servidor]**: El cruce de combate vive solo dentro del evento de promoción — no participa de ningún torneo (`tournament_matches`) ni escribe en el historial de peleas oficiales (HU-10/11), mismo aislamiento que ya decidió HU-44 CA-03 para Torneos.
- CA-05: Un candidato sin rival disponible dentro del mismo evento (número impar de candidatos, o nadie más con ese criterio) simplemente no tiene cruce registrado para ese criterio — el admin igual le carga la nota a mano, como con cualquier otro criterio.
...
**HU-16: Crear challenges** *(acotada 2026-09-12, ampliada 2026-09-14, ver nota de versión)*
Como admin, quiero crear desafíos de asistencia o peleas amistosas, para incentivar la participación de los miembros.
- CA-01: Puedo crear un desafío con nombre, descripción, tipo (asistencia o peleas amistosas), modo (meta compartida o ranking) y fechas de inicio y fin. En modo meta compartida, además defino una meta numérica.
- CA-02: Los miembros ven los desafíos activos y su progreso individual — hacia la meta en modo meta compartida, o su puesto y el podio en modo ranking.
...
- CA-05: Puedo cerrar un desafío antes de su fecha de fin y ver quiénes lo completaron (modo meta) o el podio final (modo ranking).
...
- CA-07 *(nueva 2026-09-14)*: En modo ranking, defino cuántos ganan: el top 1 (un solo ganador) o el top 3 (podio). No hay meta numérica en este modo — gana quien queda mejor posicionado al momento de cerrar el desafío, empates comparten puesto.
- CA-08 *(nueva 2026-09-14)* **[servidor]**: A diferencia del progreso agregado de un desafío meta (CA-05, visible solo para el staff), el podio de un desafío ranking es visible para cualquier miembro del dojo — es el punto del desafío, no un dato a ocultar.
...
**HU-18b: Registrar un pago presencial** *(nueva, construida 2026-09-14, DOJO-43)*
Como admin u owner, quiero registrar un pago que el miembro hizo en persona (efectivo, tarjeta o transferencia directa), para no depender de que suba un comprobante SINPE.
- CA-01: Elijo el miembro, el plan y el método (efectivo, tarjeta o transferencia); el monto es opcional y si lo dejo vacío se toma el precio del plan.
- CA-02: Al registrar, la suscripción queda activa y el comprobante nace ya aprobado, sin imagen.
- CA-03: El período se encola con la misma regla que HU-18 CA-03 (desde el vencimiento vigente, no desde hoy).
- CA-04 **[servidor]**: Solo admin u owner pueden registrar; un miembro que lo intenta recibe error.
- CA-05 **[servidor]**: No puedo registrar mi propio pago — misma regla que HU-17 CA-04 aplicada acá.
...
**HU-37: Editar mi perfil y mi foto** *(nueva — quedó pendiente en v1; acotada y cerrada 2026-09-13, DOJO-36 — [PR #32](https://github.com/MarcosZam13/CoreBase/pull/32) mergeado, ver `modulos/perfil/README.md` en el repo)*
...
- CA-04 **[servidor]**: La foto se guarda privada (bucket de Storage, nunca una URL pública) y se sirve con URL firmada de corto plazo -- mismo criterio que comprobantes/adjuntos de contenido.
...
**HU-41: Ver la app como la ve un alumno** *(nueva, evaluada y descartada 2026-09-14 -- ver nota de versión)*
