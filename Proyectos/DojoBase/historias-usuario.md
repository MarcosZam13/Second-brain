...
**HU-45: Evaluar un criterio de combate con un cruce, no un puntaje a mano** *(nueva — pedido de Marcos el 2026-09-13, tras ver la card de cruce manual de la proyección de Torneos, DOJO-31)*
Como admin, quiero que un criterio de un evento de promoción se pueda evaluar armando un cruce entre dos candidatos en vez de cargando un puntaje a mano, para que un examen con combate real se registre con la misma facilidad que un torneo.
...
- CA-03: **Resuelto (2026-09-13) — cómo se decide el ganador.** Mientras se juega el cruce llevo un marcador de puntos de ambos lados (mismos incrementos de la disciplina que ya usa Sparring/Torneos bracket, HU-44 CA-02), pero el marcador solo **sugiere** un ganador, nunca lo decide solo: elegir quién ganó sigue siendo un toque aparte del admin, igual que en Torneos. Alguien puede ir arriba en puntos y aun así perder por sumisión, KO o decisión — el puntaje importa hasta que un método puntual lo hace irrelevante.
- CA-03b: **Sin resolver — a confirmar con Marcos antes de asignar ticket.** Lo de arriba decide el ganador del cruce; falta decidir cómo ese resultado se convierte en **el puntaje del criterio** dentro de la nota ponderada de HU-13 (que tiene su propio puntaje máximo, distinto del marcador de puntos del combate): ¿ganador = puntaje máximo del criterio y perdedor = 0? ¿empate reparte la mitad? ¿el admin puede ajustarlo después de ver el resultado, o queda fijo?
...
- CA-05: Un candidato sin rival disponible dentro del mismo evento (número impar de candidatos, o nadie más con ese criterio) se califica a mano como excepción — el cruce es la vía principal para este tipo de criterio, no la única.
...
| Promociones *(HU-45 bocetada, sin ticket — ver nota v1.8 en `estado`)* | HU-12, 12b, 13, 13b, 14, 45 | RF-11 a RF-12b |
