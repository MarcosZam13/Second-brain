...
**`TournamentProjection` (K1).** Se ve desde el otro lado del gimnasio, en un televisor, por gente parada. Nada de tipografía de interfaz: tamaños de display, contraste máximo, cero elementos de navegación — resuelto con el zoom nativo del navegador (HU-38 CA-01), no con una escala de tokens aparte. Se actualiza en tiempo real — un marcador proyectado con 15 segundos de retraso se nota y desprestigia el torneo. **DOJO-31** (2026-09-13, en curso): reusa `TournamentMatchCard`/`BracketView` tal cual con `staff=false`, sin rango/franjas por participante. Ver `modulos/torneos-proyeccion/decisiones.md` en el repo.
...
*(2026-09-06/07, DOJO-14): el feed "Próximas peleas" agregado de forma aditiva sobre el scaffold que ya existía en `/panel`. El feed es horizontal con scroll, tarjetas con el ancho del token `--container-list` (320px) con el nombre del miembro y el rival apilados en líneas separadas (no en una sola fila): un primer intento con ambos nombres en una fila truncaba nombres completos a la mitad en una tarjeta angosta — corregido antes de cerrar el módulo.*

*(2026-09-06/07, DOJO-15): pantalla reconstruida por completo a pedido de Marcos ("no tenía contenido relevante"). Card de mi membresía (días restantes en grande, franja de aviso arriba de todo si vence en ≤7 días), próxima clase (la reservada o la próxima abierta con botón de inscribirme), "Te retaron" (sin `EmptyState` cuando no hay retos — ver `modulos/inicio-dashboard/decisiones.md` #1), el feed de arriba, y "El dojo hoy" (D1) para staff con comprobantes esperando y clases de hoy. El scaffold original ("El tema es un dato, no código" / "Sumar gente al dojo") se eliminó por completo.*
