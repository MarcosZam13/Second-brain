...
| Semana 7 | 15–21 sep | Coordinador + modelo de datos + un worker end-to-end en una sola máquina (caso homogéneo simple) | **Rol A hecho 2026-09-17** (probado con caso heterogéneo exitoso y caso con falla parcial, no solo el homogéneo mínimo). Repo pusheado a `github.com/MarcosZam13/plataforma-multimedia` 2026-09-18, con proceso de equipo completo (CLAUDE.md, ARCHITECTURE/SCRUM/USER_STORIES/GITFLOW, skills y subagentes propios) — falta invitar a Mario y Joseph. Choca con la Indagatoria (entrega 17 sep) — repartir bien el tiempo con Rol B/C |
| Semana 8 | 22–28 sep | Distribuir en las 3 máquinas reales, task routing a los 3 tipos de worker, `chord` para casos heterogéneos, dashboard básico, script de dataset | Choca con la I Prueba de Curso (24 sep). **PR #1 de Joseph (Rol C) mergeado a `main` 2026-09-22** tras revisión de Marcos: dataset generator, reporter de recursos, dashboard, prioridad por cola, base compartida por Postgres — con 2 bugs de la revisión ya corregidos (prioridad sin cota, `/resultados` roto para metadata) y `pool_pre_ping` agregado. Falta correr el smoke test completo con Docker levantado antes de integrar las 3 máquinas. Mario puede arrancar Rol B contra `main` |
