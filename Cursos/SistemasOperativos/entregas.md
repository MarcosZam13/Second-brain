...
| 2026-10-01 (semana 9) | [[Cursos/SistemasOperativos/apuntes/proyecto-programado-1-plataforma-multimedia\|I Proyecto Programado]] — plataforma distribuida de procesamiento multimedia por casos (v2.0) | parte del 30% | **Rol A hecho y probado (2026-09-17); repo en GitHub (`MarcosZam13/plataforma-multimedia`) con proceso de equipo completo (2026-09-18).** PR #1 de Joseph (Rol C: dataset, reporter de recursos, dashboard, prioridad de colas, base compartida por Postgres para las 3 máquinas) revisado por Marcos (Rol A) y mergeado a `main` 2026-09-22 — 2 bugs reales corregidos en la revisión (prioridad sin cota en `app/schemas.py`, `/resultados` roto para sub-tareas de metadata) más `pool_pre_ping` en el engine de Postgres. Smoke test completo (`demo_client.py` contra Redis+worker+coordinador, caso heterogéneo video+audio+metadata) corrido y en verde 2026-09-22, incluyendo verificación en vivo de los 2 fixes (metadata en `/resultados` y rechazo de prioridad fuera de rango). Mario (Rol B, ffmpeg real) puede arrancar ya contra `main`. **PR #2 de Joseph (auditoría, rebalanceador, dashboard) revisado 2026-09-24:** conflicto con `main` resuelto y fixes empujados a su rama (balanceador dejaba colas sin consumidor en el Esc 9, ignoraba nodos con CPU 0.0 y backlog con prioridad; path traversal de entrada; lock sin renovar), 65 tests + smoke en vivo en verde, detalle en `docs/AVISOS_ROL_A.md` §14 del repo y comentario en el PR. Joseph subió evidencias a `docs/evidencias/` + checklist; **PR #2 mergeado a `main` 2026-09-24** (`17f8340`, 65 tests en verde en `main`). Mario puede seguir con Rol B contra `main` |
