...
10. Al agregar una fecha de entrega nueva en `entregas.md`, ofrecer sincronizarla al calendario (MCP de Google Calendar ya conectado) o a Todoist (MCP ya conectado) según corresponda. Actualizar también el bloque de frontmatter `entregas_pendientes` del mismo `entregas.md` (agregar/quitar/marcar entregado) para que `Dashboard.md` (Dataview) se mantenga al día — si ese curso todavía no tiene el bloque, crearlo.
