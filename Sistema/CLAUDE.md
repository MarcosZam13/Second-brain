...
- `/Proyectos/{CoreBase|DojoBase|GymBase|Tacha|AsistenciasTEC|CaneleApp|COMPDES}/` — contexto y documentación de cada proyecto. CoreBase/DojoBase/GymBase son tres notas separadas pero **un solo repo** (el monorepo `corebase`): CoreBase es la plataforma y la capa compartida, los otros dos son las apps que la consumen — ver [[Proyectos/CoreBase/README|CoreBase]] antes de tocar cualquiera de los tres
- `/Sistema/aprendizaje/` — base de conocimiento de arquitectura/buenas prácticas de uso general (SOLID, TypeScript, git, hooks, etc.), distinta de `Sistema/skills/` (esos son instrucciones de comportamiento para Claude Code; esto es material para que el usuario entienda y explique el código) — ver [[Sistema/aprendizaje/README|README]]
...
2.1. **Si la tarea toca código de CoreBase, DojoBase o GymBase v2, leer primero `C:/Users/luisy/Documents/ProyectosPersonales/corebase/CLAUDE.md`.** Ese archivo es el contrato de trabajo dentro del repo (las cuatro reglas ejecutables, la estructura del monorepo, los patrones de JWT/RLS/server actions) y **no se carga solo** cuando la sesión se abre desde el vault. Sin leerlo se trabaja sin las reglas que ese repo sí verifica en el commit. El código vive en `ProyectosPersonales/corebase/`, fuera del vault, y se edita por ruta absoluta desde acá sin problema.
...
3.1. Para las features propias de un sprint de Tacha o AsistenciasTEC (dimensionarlas, construirlas en pasos explicables, guía de defensa, simulacro "modo profe"), aplicar [[Sistema/skills/defensa-de-codigo/SKILL|defensa-de-codigo]].
...
6. Para documentación técnica (README, docs, comentarios), aplicar [[Sistema/skills/professional-technical-docs/SKILL|professional-technical-docs]]. Para armar o mantener el trío ARCHITECTURE.md/SCRUM.md/HU.md de un proyecto, aplicar además [[Sistema/skills/scrum-architecture-docs/SKILL|scrum-architecture-docs]].
6.1. Para código o revisión que toque auth, permisos por rol, inputs de usuario o secrets/env vars, aplicar [[Sistema/skills/security-practices/SKILL|security-practices]].
...
10.1. `entregas.md` es el índice que sistemas externos (ej. Hermes por Telegram) consultan para responder "qué hay que hacer" de un curso — no exploran `apuntes/`, `entregables/` ni `repasos/` por su cuenta. Por eso el wikilink de cada fila tiene que apuntar siempre al contenido más útil y vigente para esa entrega, no solo al apunte fuente: si ya existe un `entregable/` o `repaso/` para esa fecha, el link va ahí (con `apuntes/` como referencia secundaria vía el "Ver también" de ese mismo archivo, nunca al revés). Cuando ese entregable/repaso se crea o cambia de estado (borrador → entregado), actualizar el link y la columna "Estado" de `entregas.md` en el mismo commit — un link a la fuente vieja deja el contenido real invisible para quien solo lee este archivo. Ver [[Sistema/skills/quiz-repaso/SKILL|quiz-repaso]] para el caso de repasos.
11. Para tareas grandes (cerrar un ticket, preparar un PR), considerar usar los subagentes en `Sistema/agents/` (code-reviewer, qa-checker, docs-writer, security-reviewer) en vez de hacer todo en una sola pasada — ver [[Sistema/agents/README|agents/README]].
