...
**Fix adicional, mismo día — dejar de confiar en el "✅" del modelo sin chequear el resultado real:** el incidente del punto 1 de arriba (ID mal transcrito → 404) se agravó porque el modelo igual reportó éxito sin mirar el JSON de la respuesta. El template ahora exige explícitamente revisar el campo `"success"` de cada script antes de confirmar, y define un mensaje distinto (`⚠️ No pude marcar...`) para cuando falla — no alcanza con pedir "revisá antes de confirmar" en general, hay que decirle qué campo mirar y qué decir en cada caso.
