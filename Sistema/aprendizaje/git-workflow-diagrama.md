...
- **Labels de estado en cada PR** (`in progress`, `waiting qa`, `qa accepted`, `qa denied`, `on hold`), sincronizados con el tablero de Jira. En el Modelo 3 de Tacha y Asistencias TEC: la PR se abre al empezar la historia, una sola `in progress` por persona, y las historias dependientes esperan en `on hold` en vez de apilar ramas. Detalle en [[Sistema/skills/gitflow-scrum/SKILL#1c. Labels de estado de PR (van con la variante de Entregables)|gitflow-scrum §1c]].
...
- **"¿Por qué abrís la PR si todavía no programaste nada?"** → Porque la PR es el registro público de quién trabaja en qué. Abierta desde el inicio (con la SPEC o un commit vacío) y con su label, el equipo ve el estado real en GitHub sin preguntar, y la historia no queda invisible hasta el último día.
- **"¿Por qué solo una PR `in progress` a la vez?"** → Para que el label diga la verdad: si tenés cinco "en progreso", ninguna lo está de verdad. Obliga a terminar (`waiting qa`) o pausar explícitamente (`on hold`) antes de saltar a otra cosa, y el tablero refleja en qué estás realmente.
- **"Tu historia depende de la de un compañero, ¿por qué no sacaste tu rama de la suya?"** → Porque si la de él cambia en QA o la rechazan, mi rama arrastra código que nunca entró a `develop`. Mi rama nace de `develop` y queda `on hold`; cuando la de él se mergea, rebaseo sobre `develop` y sigo.
